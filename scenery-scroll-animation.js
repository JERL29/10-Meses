...
// Animación para todos los grupos de luciérnagas
gsap.utils.toArray(".firefly-group").forEach(group => {
  gsap.fromTo(group, { opacity: 0, y: 400, scale: 0 }, {
    y: 120,
    scale: 1,
    opacity: 1,
    ...
    scrollTrigger: {
      ...
      scrub: 3,
      onEnter: () => {
        gsap.utils.toArray(group.querySelectorAll("circle")).forEach((item) => {
          gsap.to(item, {
            x: () => Math.random() * 20 - 10,
            y: () => Math.random() * 20 - 10,
      ...
      onLeave: () => {
        gsap.to(group, { opacity: 0, delay: 2 });
      },
    }
  });
});
